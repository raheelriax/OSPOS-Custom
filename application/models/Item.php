	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
		...
		else
		{
			...
			$this->db->select('MAX(items.per_foot_price) as per_foot_price');
			$this->db->select('MAX(items.weight_per_pipe) as weight_per_pipe');
		...
		if(!empty($search))
		{
			...
			{
				$this->db->group_start();
					$this->db->like('per_foot_price', $search);
					$this->db->or_like('weight_per_pipe', $search);
