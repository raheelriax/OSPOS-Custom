	...
	*/
	...
	public function save($item_id = -1)
	{
		...
		$item_data = array(
			...
			'per_foot_price' => $this->input->post('per_foot_price') == NULL ? '' : $this->input->post('per_foot_price'),
			'weight_per_pipe' => $this->input->post('weight_per_pipe') == NULL ? '' : $this->input->post('weight_per_pipe'),
				...
				while(($data = fgetcsv($handle)) !== FALSE)
				{
					...
					if(sizeof($data) >= 23)
					{
						$item_data = array(
							...
							'per_foot_price'		=> $data[14],
							'weight_per_pipe'		=> $data[15],
							'custom3'				=> $data[16],
							'custom4'				=> $data[17],
							'custom5'				=> $data[18],
							'custom6'				=> $data[19],
