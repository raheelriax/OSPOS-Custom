...
<?php echo form_open('items/save/'.$item_info->item_id, array('id'=>'item_form', 'enctype'=>'multipart/form-data', 'class'=>'form-horizontal')); ?>
	<fieldset id="item_basic_info">
		...
		<div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('items_per_foot_price'), 'per_foot_price', array('class'=>'required control-label col-xs-3')); ?>
			<div class='col-xs-4'>
				<div class="input-group input-group-sm">
					<?php if (!currency_side()): ?>
						<span class="input-group-addon input-sm"><b><?php echo $this->config->item('currency_symbol'); ?></b></span>
					...
					<?php echo form_input(array(
							...
							'id'=>'per_foot_price',
							'class'=>'form-control input-sm',
							'value'=>to_currency_no_money($item_info->per_foot_price))
							);?>
					<?php if (currency_side()): ?>
						<span class="input-group-addon input-sm"><b><?php echo $this->config->item('currency_symbol'); ?></b></span>
					<?php endif; ?>
		...
		<div class="form-group form-group-sm">
				<?php echo form_label($this->lang->line('items_weight_per_pipe'), 'weight_per_pipe' . $key, array('class'=>'required control-label col-xs-3')); ?>
				<div class='col-xs-4'>
					<?php echo form_input(array(
							'name'=>'weight_per_pipe' . $key,
							'id'=>'weight_per_pipe' . $key,
							'class'=>'required weight_per_pipe form-control',
							'value'=>to_quantity_decimals($item_info->weight_per_pipe))
							);?>
				</div>
			</div>
		...
		?>
			<div class="form-group form-group-sm">
				<?php echo form_label($this->lang->line('items_quantity').' '.$location_detail['location_name'], 'quantity' . $key, array('class'=>'required control-label col-xs-3')); ?>
				<div class='col-xs-4'>
					<?php echo form_input(array(
							'name'=>'quantity_' . $key,
							'id'=>'quantity_' . $key,
...
$(document).ready(function()
{
	...
	$('#item_form').validate($.extend({
				...
				success: function(response) {
					...
					if (stay_open)
					{
						...
						$(':text, :password, :file, #description, #item_form').not('.quantity, #reorder_level, #tax_name_1,' +
							'#tax_percent_name_1, #reference_number, #name, #cost_price, #unit_price,#per_foot_price, #weight_per_pipe, #taxed_cost_price, #taxed_unit_price').val('');
		...
		messages:
		{
			...
			per_foot_price:
			{
				required: "<?php echo $this->lang->line('per_foot_price_required'); ?>",
				number: "<?php echo $this->lang->line('per_foot_price_number'); ?>"
			},
			weight_per_pipe:
			{
				required: "<?php echo $this->lang->line('weight_per_pipe_required'); ?>",
				number: "<?php echo $this->lang->line('weight_per_pipe_number'); ?>"
			},
			<?php
			foreach($stock_locations as $key=>$location_detail)
			{
